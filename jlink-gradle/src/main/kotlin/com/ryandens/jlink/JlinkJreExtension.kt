...
abstract class JlinkJreExtension
    ...
    constructor(objects: ObjectFactory) {
        ...
        val variants: NamedDomainObjectContainer<JlinkVariant> =
            objects.domainObjectContainer(JlinkVariant::class.java) { name ->
...
abstract class JlinkVariant
    ...
    constructor(val name: String) {
        ...
        /**
         * Name of the [com.ryandens.jlink.tasks.JlinkJreTask] that will produce the JRE for this variant.
         */
        val jlinkTaskName get() = "jlink${name.capitalized()}"

        /**
         * Name of the [org.gradle.api.Task] that will download and extract the JDK for this variant.
         */
        val downloadTaskName get() = "download${name.capitalized()}Jdk"

        init {
            jdkDependencyCoordinates.convention(
                version.map { variantVersion ->
                    val majorVersion =
                        Pattern.compile("^\\D*(\\d+)").matcher(variantVersion).run {
                            find()
                            group(1)
                        }
                    val embeddedVersion = variantVersion.trimStart { !it.isDigit() }.replace('+', '_')
                    "temurin$majorVersion-binaries:OpenJDK${majorVersion}U-jdk_${name}_hotspot_$embeddedVersion" +
                        ":$variantVersion@${operatingSystem.get().archiveExtension}"
                },
            )
        }
    }
